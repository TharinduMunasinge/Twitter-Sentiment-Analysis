...
posTagger <- function(corpus){
	x<-treetag(corpus, treetagger='manual', lang='en', TT.options=list(path='~/workspace/tree-tagger', preset='en'))
...
mergeSimilarRows <- function(q){
p <- data.frame()
for (i in 1:nrow(q)){
	if (nrow(p)>0){
		if(q$token[i] %in% p$token){
			index <- match(q$token[i],p$token)
			p$count[index] <- p$count[index] + q$count[i]
			p$occurance[index] <- p$occurance[index] + q$occurance[i]
		}else{
			p <- rbind(p,data.frame(token=q$token[i],count=q$count[i],occurance=q$occurance[i]))
		}
	}else{
		p <- rbind(p,data.frame(token=q$token[i],count=q$count[i],occurance=q$occurance[i]))
	}
...
extractWordBag <- function(corpus){
	wordBag <- data.frame()
	for (i in 1:length(corpus)){
		write.csv(tolower(corpus[i]),file="tweet")
		tagTree <- posTagger("tweet")
		print(i)
		print(corpus[i])
		if (nrow(wordBag)>0){
				wordBag$temp <- 0				
		...
		for(j in 1:nrow(tagTree)){
			
			token <- tagTree$Token[j]
			if (!(tagTree$Tag[j] %in% c("punctuation","comma","fullstop"))){
				if (nrow(wordBag)>0){
					if(token %in% wordBag$token){
						index <- match(token,wordBag$token)
						wordBag$count[index] <- wordBag$count[index] + 1
						wordBag$temp[index] <- 1
						
					}else{
						newWord <- data.frame(token=token,count=1,occurance=0,temp=1)
						wordBag <- rbind(wordBag, newWord)
					}
				}else{
						newWord <- data.frame(token=token,count=1,occurance=0,temp=1)
						wordBag <- rbind(wordBag, newWord)
		...
		wordBag$occurance <- wordBag$occurance + wordBag$temp
		#print(nrow(wordBag))
	}
	wordBag$temp <- NULL
  wordBag <- wordBag[-as.numeric(row.names(wordBag[wordBag$count == 1,])),]
	wordBag <- wordBag[-as.numeric(row.names(wordBag[wordBag$occurance == 1,])),]
  return (wordBag)
